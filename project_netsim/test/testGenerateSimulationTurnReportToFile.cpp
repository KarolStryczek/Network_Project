...
/**
 * Test whether data is saved correctly.
 */
TEST(GenerateReportToFile, SimulationTurn) {
    Factory factory;
    ElementID ramp_id = 1;
    ...
    generateSimulationTurnReportToFile("TestEmpty.txt", factory);
    EXPECT_TRUE(generateSimulationTurnReportToFile("TestEmpty.txt", factory));

    FILE * testing_ptr;
    FILE * pattern_ptr;
    testing_ptr = fopen("TestEmpty.txt", "r");
    pattern_ptr = fopen("reportSimulationTest.txt", "r");
    ...
    if (testing_ptr == nullptr) {
        std::cout << "Error while opening reportSimulationTest.txt during IOTest-Save implementing." << "\n";
    }
